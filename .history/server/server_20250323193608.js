...
const fs = require('fs');
const { JWT_SECRET, PORT, ADMIN } = require('./config');
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    // 首先检查是否是配置文件中定义的管理员用户
    if (ADMIN && ADMIN.users) {
      const adminUser = ADMIN.users.find(user => user.username === username);
      if (adminUser && adminUser.password === password) {
        // 如果是配置中的管理员用户，直接生成JWT令牌
        const token = jwt.sign(
          { id: 0, username: username, isAdmin: true }, // 使用id=0作为标记
          JWT_SECRET,
          { expiresIn: '24h' }
        );
