...
const ProfileModal = ({ isOpen, onClose, username }) => {
  ...
  const handleViewProject = (project) => {
    // 保存选中的项目到localStorage以便Gallery页面可以显示它
          ...
          <div className="profile-section">
            <div className="section-header">
              ...
              <button 
                className="add-experiment-button"
                onClick={() => {
                  // 在localStorage中设置视图模式为gallery（交流社区）
                  localStorage.setItem('galleryViewMode', 'gallery');
                  navigate('/gallery');
                }}
              >
                <FaRocket /> 去发布作品
            ...
            <div className="user-projects-list">
              {userProjects.length > 0 ? (
                userProjects.map(project => (
                  <div key={project.id} className="user-project-item" onClick={() => handleViewProject(project)}>
                    <div className="user-project-image">
                      <img 
                        ...
                        onError={(e) => {
                          ...
                          e.target.src = 'https://via.placeholder.com/150x100?text=图片加载失败';
                        }}
                      />
                    </div>
                    <div className="user-project-info">
                      ...
                      <div className="user-project-meta">
                        ...
                        <span>点赞: {project.likes}</span>
                        <span>评论: {(project.comments || []).length}</span>
                      </div>
                ...
                <div className="empty-projects">
                  <p className="empty-message">您还没有发布过作品</p>
                  <p className="empty-message-sub">前往作品库发布您的第一个作品吧！</p>
                </div>
              )}
            </div>
          </div>
