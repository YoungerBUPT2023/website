...
const ProfileModal = ({ isOpen, onClose, username }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // 添加自定义事件监听器，监听作品变化
      window.addEventListener('projectsUpdated', loadUserProjects);
    ...
    return () => {
      ...
      window.removeEventListener('projectsUpdated', loadUserProjects);
    };
  }, [isOpen, username]);
  
  // 监听localStorage变化，以便检测全局项目数据的更改
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === 'globalProjects' || e.key === null) {
        loadUserProjects();
      }
    };
  ...
  const loadUserProjects = () => {
    try {
      ...
      // 检查是否与当前状态不同，只有不同时才更新状态，避免不必要的渲染
      if (JSON.stringify(userPublishedProjects) !== JSON.stringify(userProjects)) {
  ...
  const handleDeleteProject = (projectId) => {
    try {
      // 从localStorage获取全局项目数据
      const globalProjects = JSON.parse(localStorage.getItem('globalProjects') || '[]');
      
      // 过滤掉要删除的项目
      const updatedProjects = globalProjects.filter(project => project.id !== projectId);
