...
function Navbar() {
  ...
  useEffect(() => {
    ...
    // 监听从Gallery组件触发的打开个人资料模态框的事件
    const handleProfileModalEvent = (event) => {
      if (event.detail && event.detail.username) {
        // 如果事件中包含用户名，则显示该用户的个人资料
        console.log('接收到打开个人资料模态框事件', event.detail.username);
        setShowProfileModal(true);
      }
    };
    
    window.addEventListener('openProfileModal', handleProfileModalEvent);
    
    return () => {
      window.removeEventListener('storage', checkLoginStatus);
      window.removeEventListener('openProfileModal', handleProfileModalEvent);
    };
  }, []);
