...
const Gallery = () => {
  ...
  useEffect(() => {
    ...
    // 检查localStorage中是否设置了视图模式
    const savedViewMode = localStorage.getItem('galleryViewMode');
    if (savedViewMode) {
      setViewMode(savedViewMode);
      // 使用后清除，避免影响下次正常访问
      localStorage.removeItem('galleryViewMode');
    }
  }, []);
    ...
    const fetchProjects = async () => {
      try {
        ...
        if (selectedProjectId) {
          // 查找对应的项目
          const projectToShow = data.find(p => p.id.toString() === selectedProjectId) || 
                               // 如果API返回的数据中没有，则从localStorage中查找
                               JSON.parse(localStorage.getItem('globalProjects') || '[]')
                                 .find(p => p.id.toString() === selectedProjectId);
          ...
          // 清除localStorage中的selectedProjectId
        ...
        const selectedProjectId = localStorage.getItem('selectedProjectId');
        if (selectedProjectId) {
          const projectToShow = savedProjects.find(p => p.id.toString() === selectedProjectId);
          if (projectToShow) {
            setSelectedProject(projectToShow);
            setShowDetailModal(true);
          }
