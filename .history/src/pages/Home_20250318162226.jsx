...
function FeatureCard({ icon, title, desc, longDesc, path, color, index, modelPath }) {
  ...
  const backgroundImages = [
    '/backgrounds/lab-design.jpg',
    '/backgrounds/vr-control.jpg',
    '/backgrounds/gallery.jpg',
    '/backgrounds/coding.jpg'
  ];
  ...
  const processLongDesc = (descArr) => {
    return descArr.map((paragraph, i) => {
      if (paragraph.startsWith('【')) {
        ...
        // 检查后续段落是否也是列表项
        for (let j = i + 1; j < descArr.length; j++) {
          if (descArr[j].startsWith('•')) {
            listItems.push(descArr[j].replace('• ', ''));
            // 将已处理的列表项从原数组中标记为已处理
            descArr[j] = null;
          } else {
            break;
      ...
      return null;
    }).filter(item => item !== null); // 过滤掉已处理的列表项
  };
