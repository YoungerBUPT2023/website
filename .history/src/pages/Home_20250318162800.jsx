...
function Earth({ scrollY }) {
  ...
  useFrame(({ clock }) => {
    ...
    if (groupRef.current) {
      // 添加轻微的浮动效果
      const floatY = Math.sin(elapsedTime * 0.3) * 0.2;
      const floatX = Math.cos(elapsedTime * 0.2) * 0.1;
      
      // 固定位置，只有轻微浮动，向右偏移一点
      groupRef.current.position.x = 3.0 + floatX;
      groupRef.current.position.y = floatY;
      groupRef.current.position.z = -2.0;
      
      // 保持固定大小
      const scale = 2.2 + Math.sin(elapsedTime * 0.2) * 0.05;
