...
- REST API支持第三方应用集成`,
  ...
  const handleUpload = async (e) => {
    ...
    if (!uploadData.title.trim() || !uploadData.description.trim() || !selectedFile) {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        const projectData = {
          ...
          comments: [],
          // 添加详细描述等信息
          detailedDescription: uploadData.description, // 如果没有单独的详细描述字段，使用描述作为详细描述
          equipment: '标准设备',
          requirements: '无特殊要求',
          collaborators: []
        };
        ...
        try {
          // 模拟API请求
          // 由于没有真正的后端API，我们直接处理本地存储
          const newProjectId = Date.now(); // 生成唯一ID
          const newProject = {
            ...projectData,
            id: newProjectId,
          };
  ...
  const handleLike = async (e, projectId) => {
    ...
    try {
      ...
      setProjects(prevProjects => {
        ...
        // 触发自定义事件，通知其他组件（如ProfileModal）项目数据已更新
        window.dispatchEvent(new CustomEvent('projectsUpdated'));
