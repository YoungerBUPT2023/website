  ...
  useFrame(({ clock }) => {
    ...
    if (groupRef.current) {
      ...
      if (scrollProgress < 1) {
        ...
        targetX = 6 - progress * 0; // 保持X不变
        targetY = 4 - progress * 12; // 从上到下
        targetScale = 1.5;
...
function FeatureCard({ icon, title, desc, longDesc, path, color, index, modelPath }) {
  ...
  return (
    ...
    >
      {/* 地球位置指示器 */}
      {index === 0 && (
        <div style={{...earthIndicatorStyle, top: '20px', right: '20px'}} />
      )}
      {index === 1 && (
        <div style={{...earthIndicatorStyle, bottom: '20px', right: '20px'}} />
      )}
      {index === 2 && (
        <div style={{...earthIndicatorStyle, bottom: '20px', left: '20px'}} />
      )}
      {index === 3 && (
        <div style={{...earthIndicatorStyle, top: '20px', left: '20px'}} />
      )}
