...
- REST API支持第三方应用集成`,
  ...
  const handleUpload = async (e) => {
    ...
    if (!uploadData.title || !uploadData.description || !uploadData.imageUrl) {
      alert('请填写所有必填字段');
      return;
    }

    // 表单验证
    if (uploadData.title.length < 2) {
      alert('标题太短，请至少输入2个字符');
      return;
    }

    if (uploadData.description.length < 10) {
      alert('描述太短，请至少输入10个字符');
      return;
    }
    ...
    try {
      ...
      setProjects(prevProjects => {
        const updatedProjects = [data.project, ...prevProjects];
