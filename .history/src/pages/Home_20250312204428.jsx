...
function Earth({ scrollY }) {
  ...
  useFrame(({ clock }) => {
    ...
    if (groupRef.current) {
      ...
      groupRef.current.position.x = scrollProgress * 6; // 从中间向右移动
      groupRef.current.position.y = -8 + scrollProgress * 12; // 从下向上移动，初始位置更低
      
      // 缩放效果 - 随滚动逐渐缩小
      const scale = 4 - scrollProgress * 1.5;
      groupRef.current.scale.set(scale, scale, scale);
      
      // 旋转效果 - 随滚动稍微调整角度
      groupRef.current.rotation.x = 0.2 + scrollProgress * Math.PI * 0.1;
