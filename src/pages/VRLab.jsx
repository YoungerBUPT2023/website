...
function VRLab() {
  ...
  const defaultMessages = {
    1: [
      { id: 1, username: '张老师', content: '欢迎来到天宫课堂球形火焰实验讨论区', timestamp: new Date().toLocaleTimeString() },
      { id: 2, username: '李同学', content: '老师，为什么在太空中火焰会呈现球形？', timestamp: new Date().toLocaleTimeString() },
      { id: 3, username: '张老师', content: '这是因为在微重力环境下，没有浮力对流的影响，火焰可以均匀地向四周扩散，形成完美的球形。大家要仔细观察火焰的形状变化和燃烧特性。', timestamp: new Date().toLocaleTimeString() }
    ],
    2: [
      { id: 1, username: '张老师', content: '欢迎来到天宫课堂动量守恒实验讨论区', timestamp: new Date().toLocaleTimeString() },
      { id: 2, username: '王同学', content: '老师，为什么要在太空中做动量守恒实验？', timestamp: new Date().toLocaleTimeString() },
      { id: 3, username: '张老师', content: '在太空微重力环境中，几乎没有摩擦力和重力的影响，可以更纯粹地观察到动量守恒定律。大家要注意观察碰撞前后物体的运动状态。', timestamp: new Date().toLocaleTimeString() }
    ],
    3: [
      { id: 1, username: '张老师', content: '欢迎来到天宫课堂陀螺实验讨论区', timestamp: new Date().toLocaleTimeString() },
      { id: 2, username: '赵同学', content: '老师，太空中的陀螺运动和地面上有什么不同？', timestamp: new Date().toLocaleTimeString() },
      { id: 3, username: '张老师', content: '在微重力环境下，陀螺的运动不受重力影响，可以更清晰地观察到角动量守恒现象。请大家重点关注陀螺的进动和章动现象。', timestamp: new Date().toLocaleTimeString() }
    ]
  };
  ...
  useEffect(() => {
    ...
    const handleVideoMessage = (event) => {
      if (event.data.type === 'progress') {
        const { time, experimentId, step } = event.data.data;
        if (step !== undefined && step !== currentExperimentStep) {
          setCurrentExperimentStep(step);
        }
      }
    };
    
    window.addEventListener('message', handleVideoMessage);
    const initialCourse = coursesList[0];
    setSelectedCourse(initialCourse);
    setMessages(defaultMessages[initialCourse.id]);
    checkAdminStatus();
  ...
  const handleSelectCourse = (course) => {
    ...
    // 更新讨论消息
    setMessages(defaultMessages[course.id]);
  };
      ...
      <div className="experiment-lab">
        ...
        <div className="lab-container">
          <VideoPlayer
            ...
            experimentContents={experimentContents}
          />
...
function VideoPlayer({ title, selectedCourse, onSelectCourse, coursesList, experimentContents }) {
  ...
  const handleTimeUpdate = () => {
    if (videoRef.current && selectedCourse) {
      ...
      // 更新实验步骤
      const steps = experimentContents[selectedCourse.id]?.steps;
      if (steps) {
        ...
        if (newStep !== -1 && newStep !== currentExperimentStep) {
          setCurrentExperimentStep(newStep);
        }
        ...
        window.postMessage({
          ...
          data: {
            time: currentTime,
            experimentId: selectedCourse.id,
            step: newStep
          }
        }, '*');
      }
    ...
    <div className="video-player">
      <div className="video-header" style={{ position: 'relative', zIndex: 100 }}>
        ...
        <div className="video-info">
          ...
          <div className="info-item">
            <span className="info-label">实验时间:</span>
            <span className="time-display">{formatCurrentTime()}</span>
          </div>
          <div className="info-item">
            <span className="info-label">视频进度:</span>
            <span className="time-display">{formatVideoTime(currentTime)} / {formatVideoTime(duration)}</span>
          </div>
                ...
                {coursesList.map((course, index) => (
                  ...
                  >
                    ...
                    <div className="playlist-item-info">
                      ...
                      <div className="playlist-item-duration">{selectedCourse?.id === course.id ? formatVideoTime(duration) : '--:--'}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="video-description">
        ...
        <p style={{ 
          ...
          borderTop: '1px solid rgba(97, 218, 251, 0.15)',
          paddingTop: '8px',
          lineHeight: '1.4'
        }}>
          特别鸣谢：本实验演示视频源自B站UP主"我们的空间站"。由于实验环境正在筹备中，目前使用天宫课堂的精彩内容为大家展示太空实验的独特魅力。
        </p>
      </div>
    </div>
  );
